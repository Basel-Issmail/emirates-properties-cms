...
@Component({
  selector: 'ep-amenity-category-table',
  templateUrl: './amenity-category-table.component.html',
  styleUrls: ['./amenity-category-table.component.scss']
})
export class AmenityCategoryTableComponent implements AfterViewInit {
    ...
    merge(this.sort.sortChange, this.paginator.page, this.selectedTab, this.changedData)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.sharedCrudService.getData(AmenityCategoryApis.getData, { tab: this.tab, sortCol: this.sort.active, sortDir: this.sort.direction, page: (this.paginator.pageIndex + 1), pageSize: this.pageSize });
        }),
        map(data => {
  ...
  changeStatus(selected, attribute, value) {
    this.sharedCrudService.changeAttribute(AmenityCategoryApis.changeAttribute, selected, attribute, value).subscribe(response => {
      this.changedData.next();
    })
  }
