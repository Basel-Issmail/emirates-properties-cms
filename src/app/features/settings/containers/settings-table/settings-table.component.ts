...
@Component({
  selector: 'ep-settings-table',
  templateUrl: './settings-table.component.html',
  styleUrls: ['./settings-table.component.scss']
})
export class SettingsTableComponent implements AfterViewInit {
  displayedColumns: string[] = ['select', 'name', 'ident', 'content', 'actions'];
  columns = {
    cols: ['select', 'name', 'ident', 'content', 'actions'],
    select: { isShown: true, label: '', canHide: false },
    name: { isShown: true, label: 'Name', canHide: true },
    ident: { isShown: true, label: 'Ident', canHide: true },
    content: { isShown: true, label: 'Content', canHide: true },
    ...
    merge(this.sort.sortChange, this.paginator.page, this.selectedTab, this.changedData)
      .pipe(
        ...
        switchMap(() => {
          ...
          return this.sharedCrudService.getData(SettingsApis.getData, { tab: this.tab, sortCol: this.sort.active, sortDir: this.sort.direction, page: (this.paginator.pageIndex + 1), pageSize: this.pageSize });
        }),
        map(data => {
  ...
  delete(selected) {
    this.sharedCrudService.delete(SettingsApis.delete, selected).subscribe(response => {
